...
void printDLR(FILE *stream, treeNode *root){
	if(root == NULL) return;
	printNode(stream, root);
	printDLR(stream, root->left);
	printDLR(stream, root->right);
}

void printRDL(FILE *stream, treeNode *root){
	if(root == NULL) return;
	printLDR(stream, root->right);
	printNode(stream, root);
	printLDR(stream, root->left);	
...
treeNode *getParent(treeNode *root, treeNode *node){
  ...
  if (getParent(root->right, node) != NULL) return getParent(root->right, node);
  else return getParent(root->left, node);
}

void copyData(treeNode *a, treeNode *b){
  strcpy(a->data.name, b->data.name);
  strcpy(a->data.tele, b->data.tele);
  strcpy(a->data.email, b->data.email);
}

void deleteNode(treeNode **root, char *email){
  treeNode *deletingNode = searchEmail(*root, email);
  treeNode *altNode = (treeNode *)malloc(sizeof(treeNode));
  if(deletingNode->right == NULL && deletingNode->left == NULL){
    deletingNode = NULL;
    return;
  }
  else if(deletingNode->right == NULL && deletingNode->left != NULL){
    altNode = findMax(deletingNode->left);
  }
  else{
    altNode = findMin(deletingNode->right);
  }
  copyData(deletingNode, altNode);
  treeNode *altParent = getParent(*root, altNode);
  if(altParent->right == altNode) altParent->right = altNode->right;
  else if(altParent->left == altNode) altParent->left = altNode->left;
  free(altNode);
}
